...
* **Paired requirement — deterministic sort/encode, not just author filtering:** "did a peer actually change this" is only a meaningful question if identical content is guaranteed to compare equal to itself. `MembershipSnapshot.membersJson`/`TacticalSnapshot.tacticalJson` (the `*_ls` wire strings this data ultimately also feeds — see [`COMPANION_DATA_SYNC_MODEL.md` §3](COMPANION_DATA_SYNC_MODEL.md#3-merge-engine--conflict-resolution-rules)) are compared via raw string `==`, so the producing side must emit a canonical, stable ordering: `GameStateManager.syncMembershipToWatchConnectivity` sorts the roster by `id` and `syncTacticalToWatchConnectivity` sorts indicators by `timestamp`, and both use `JSONEncoder.outputFormatting = .sortedKeys` (unspecified per-object key order is otherwise a genuine source of two encodes of identical content comparing unequal). Without this, self-echo filtering alone is insufficient — a filtered-clean re-decode can still register as "changed" purely from re-encoding nondeterminism, which was observed as a permanent 1 Hz retransmission loop that never converged.
...
### F. Display Classification: Membership × Telemetry Confirmation Gate
...
* **Problem this replaces:** Room/Membership (§5.B, `/r/{roomId}/m/{memberId}`) and Remote Player Telemetry (`/p/{roomId}/{memberId}`) are two independent channels that can arrive in either order — the join path's roster write (`publishMemberToFirebase`) is fire-and-forget with no completion/retry, so a peer's telemetry frequently reaches this device before that peer's roster row does. `updateMember(with:in:)` used to create a placeholder `SquadMember` from telemetry alone (empty `callsign` when unknown), and that placeholder was classified/colored immediately — an unresolved identity has no clan tag, so it read as "different team" (blue) until the roster fetch resolved the real callsign and flipped it to "same team" (green). That flip was user-visible on the watch as a remote player rendering blue for a few seconds, then suddenly green.
* **Superseding fix — reject rather than stub:** `updateMember(with:in:)` no longer creates that placeholder at all — `guard var member = members[packet.memberId] else { return }` rejects telemetry for any member id not already present in `activeRoom.members` (i.e. not yet established by the roster channel), instead of fabricating one with an empty callsign to be corrected later. This closes the wrong-color flip at its source rather than papering over it with the confirmation gate below, and eliminates the separate `knownCallsigns` cache that previously existed purely to backfill a stub's callsign after the fact (removed entirely — see `CLAUDE.md` rule 3). The two-set confirmation gate immediately below remains in effect for the distinct, still-real case it was never solely responsible for: a member whose roster row already exists but who hasn't sent any telemetry yet.
* **Rule — a member must be confirmed via BOTH channels before being displayed or classified at all:** `FirebaseSyncManager.isMemberConfirmed(_:)` is the single gate — `observedRosterMemberIds.contains(id) && observedTelemetryMemberIds.contains(id)`. Failing either check excludes the member from `GameStateManager.otherSquadMembers` entirely; it does not fall back to a guessed/default classification. `otherSquadMembers` is the one list every view (`RadarMapView`, `StandardMapView`, `TacticalMKMapView`) renders and classifies (`isSameClan`) from, so gating it once in `updateOtherSquadMembers` covers all of them — no per-view duplication.
* **Why two independent sets instead of inferring from `activeRoom.members`:** now that telemetry can only update an already-roster-established member (see the superseding fix above), `activeRoom.members`'s keys alone *would* correctly identify roster-confirmed members — but `observedTelemetryMemberIds` still can't be inferred from it, since a member can be roster-confirmed while having sent zero telemetry (nothing to distinguish "confirmed but silent" from "confirmed and actively transmitting" by looking at `activeRoom.members` alone). `observedRosterMemberIds` is maintained only from roster-sourced writes (`mergeRemoteMembers`'s wholesale replace on every `/r/{roomId}/m` snapshot, plus a per-id insert on a successful one-shot `fetchMemberDetails`); `observedTelemetryMemberIds` is maintained from *either* telemetry source, direct or relayed (see next bullet), independent of whether that telemetry was actually accepted into `activeRoom.members`.
* **`observedTelemetryMemberIds` must track both telemetry sources identically, not just this device's own listener.** Per `COMPANION_DATA_SYNC_MODEL.md` §0, `*_hs` is two independent unidirectional, unmerged streams — a device without its own `/p/{roomId}` listener attached (either the Phone while the Watch holds network ownership, or symmetrically the Watch while the Phone holds it, §5.C) instead receives the same telemetry relayed via whichever `*_hs.remotePlayerTelemetryJson` is inbound to it (`w2p_hs` for the Phone, `p2w_hs` for the Watch — read inline by role in `WatchConnectivityManager.handleIncomingApplicationContext`, `WatchConnectivityManager.swift:499,507-508`, which fires `onHighSpeedTelemetryReceived`), and both sources write into the identical destination, `activeRoom.members`, via `updateMember(with:in:)`. `handleTelemetryChildUpsert` (the direct `/p/{roomId}` listener path) inserts into `observedTelemetryMemberIds` on every child event; `validateAndProcessPackets` (the relay entry point, called from `onHighSpeedTelemetryReceived` on either platform) now does the same for every packet it processes, unconditionally (regardless of whether the packet itself passes freshness) — mirroring the direct-listener path exactly, and fixed once in the shared function rather than per-caller. Before this, `observedTelemetryMemberIds` was populated only by the direct listener, so a device correctly relying on the relay (because it has no listener of its own attached, by design) could never satisfy `isMemberConfirmed` for *any* remote member — the roster (which reads `activeRoom.members` directly, bypassing this gate) showed them fine, but the map, gated by `isMemberConfirmed`, never did.
* **Trade-off, accepted:** a brand-new joiner is briefly invisible (rather than briefly wrong-colored) between their telemetry arriving and their roster row confirming — bounded by the same retry loop that already exists (every telemetry event re-checks and re-fetches an unresolved roster row), not by any new timeout logic.
