...
* **Why two independent sets instead of inferring from `activeRoom.members`:** `activeRoom.members` is written by *both* channels (telemetry's placeholder-creation and the roster merge), so its keys alone can't distinguish "confirmed roster row" from "we invented an entry because telemetry arrived first." `observedRosterMemberIds` is maintained only from roster-sourced writes (`mergeRemoteMembers`'s wholesale replace on every `/r/{roomId}/m` snapshot, plus a per-id insert on a successful one-shot `fetchMemberDetails`); `observedTelemetryMemberIds` is maintained from *either* telemetry source, direct or relayed (see next bullet). Neither set is ever written to from the roster channel.
