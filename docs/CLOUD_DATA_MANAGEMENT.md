...
| **§4 Refresh Heartbeat** | `refreshIntervalMultiplier` | `10.0` ($10 \times T$) (`ConstantBandwidth`) | Scheduled refresh heartbeat cadence when player is stationary / delta-gated |
...
* **Pre-Uplink Gating:** Evaluated by Dead Reckoning & Delta Gating (movement $\ge 3.5\text{ m}$ or $\Delta\text{HR} \ge 12\text{ BPM}$, with a scheduled refresh heartbeat interval $10 \times T$).
...
* **Mechanism:** High-speed stream strictly via `WCSession.sendMessage`, gated on `isReachable` with no fallback (see `WatchConnectivityManager.advertisePhoneHighSpeed`/`advertiseWatchHighSpeed`), and low-speed snapshots via `WCSession.updateApplicationContext`. Watch wins equal-timestamp ties. Rolling `sync_ts` retransmission is **deliberately not** gated on `WCSession.isReachable` — see [`COMPANION_DATA_SYNC_MODEL.md §3`](COMPANION_DATA_SYNC_MODEL.md#3-merge-engine--conflict-resolution-rules) for why: `isReachable` only reflects live-messaging availability (foreground / high-priority background) and is known to read `false` even during an active Watch workout session, while `updateApplicationContext` is designed to keep working regardless of reachability via the system WatchConnectivity daemon. Gating on it was tried and reverted — it risks silently stalling sync to a backgrounded-but-active companion for a negligible power saving.
* **`*_ls` vs `*_hs` are structurally different channels, not two flavors of one pattern** — see [`COMPANION_DATA_SYNC_MODEL.md §0`](COMPANION_DATA_SYNC_MODEL.md#0-two-structurally-different-channels--do-not-cross-the-streams). `*_ls` is a bidirectionally-shared, timestamp-merged variable — three independent instances (`Room`, `Tactical`, `Other`), each with its own `WatchConnectivityManager.local{Room,Tactical,Other}` as the single source of truth on each device (no `GameStateManager` shadow copies, and no single shared `localLS`). `*_hs` (`p2w_hs`/`w2p_hs`) is two independent one-way streams — single writer, single reader, no merge — and its HR/telemetry-source consumption rules (§2 of that doc) key off the exact same `active_until` comparison as the cloud-client role decision below, not an independent check.
...
* **Gated Listener Attachment:** `GameStateManager.evaluateListenerGate()` runs on both platforms with differentiated logic (structurally mirrored by, but independently evaluated from, the Uplink/ownership gate in §5.C below — see `hasNetworkOwnership`). It requires `login_cycle != inactive` (`host_active OR join_active` — the single synced source of truth for "did we actually choose to host/join," not a second locally-derived "session active" flag that could drift from it), then attaches based on platform role:
  * **Watch:** `login_active AND (app_active OR (watch.Time < p2w_hs.active_until))` (`loginActive && (appActive || peerLeaseActive)`).
  * **Phone:** `login_active AND app_active AND (phone.time > w2p_hs.active_until)` (`loginActive && appActive && !peerLeaseActive`). The Phone only attaches listeners when looking at the app AND the Watch has stepped down / its lease expired.
  * Gating on `login_cycle` rather than `isTacticalSessionActive` matters at the exact moment a companion adopts a session (`GameStateManager.adoptCompanionSession`, driven by an incoming `host_active`/`join_active` LS convergence): `login_cycle` is already true by construction at that instant (it's what triggered the adoption), so the freshly-attached listener isn't at risk of being torn down by this gate before its first `.value` snapshot arrives, the way it would be if the gate depended only on `app_active`/lease timing that may not have caught up yet.
...
* **Current implementation status: COMPLIANT.** `firebase-ios-sdk` is an SPM dependency (both `RadarMap.xcodeproj` via `generate_xcodeproj.py` and `Package.swift`), and all three downstream channels are implemented on top of it via the `RTDBTransport` protocol ([RTDBTransport.swift](../RadarMap/Managers/RTDBTransport.swift)), whose production implementation `FirebaseRTDBTransport` wraps `DatabaseReference`. `FirebaseSyncManager.startTelemetryPolling(roomId:)` / `stopTelemetryPolling()` are the gated attach/detach entry points, called from `GameStateManager.evaluateListenerGate()` (see the Gated Listener Attachment bullet above), calling `attachRealtimeListeners(roomId:)` / `detachRealtimeListeners()`: telemetry uses `.childAdded` / `.childChanged` / `.childRemoved` on `p/{roomId}` (per-child deltas, not whole-subtree re-transmission), tactical and room/membership each use `.value` on `t/{roomId}` / `r/{roomId}` (matching their existing whole-node merge semantics). Since these listeners fire immediately on attach with current state and again on every subsequent change, `startTelemetryPolling` does not also perform a separate instant one-shot fetch (see §7.C — that redundant REST-era fetch was removed). `fetchRemoteTelemetry(roomId:)` and `fetchTacticalIndicators(roomId:)` remain as one-shot read-and-apply methods (via `transport.getValue`, the SDK's one-shot read) for `fetchRoomDetails(roomId:)`'s initial load and explicit wake-burst refreshes, sharing their canonicalize/`Set()` logic with the persistent listeners via `applyTacticalSnapshot`/`applyRoomSnapshot`. `applyRoomSnapshot` is the pipeline stage that populates room-level metadata — `hst`/`id`/`pin`/`cap`/`mti`/`room_ts`, not per-member fields — alongside the roster from the same `/r/{roomId}` node; this metadata now writes straight into the single synced `Room` structure via `Set()` (see [`COMPANION_DATA_SYNC_MODEL.md` §0/§2](COMPANION_DATA_SYNC_MODEL.md)) rather than into a separate `FirebaseSyncManager`-only store, so there is exactly one place — `Room` — where a companion device's `hst`/`cap`/`mti` ever live, not a Firebase-side copy plus a WCSession-side copy needing to be kept in sync. (Historically, before this collapse, this same field set lived only in `FirebaseSyncManager.activeRoom` and had to be explicitly re-merged onto that store on every companion join or it would stick at a placeholder value for the life of the session — the collapse to a single `Set()`-only `Room` store removes that failure mode entirely, not just patches it.)

#### Self-Echo Filtering on SDK Listener Callbacks: Per-Domain, Not Universal
* **Cause:** Once uploads also move to the SDK (§5.C) and ride the same shared connection as these listeners, the SDK's local synchronized cache means a listener covering a path you also write to (e.g. a room-level `/p/{roomId}` listener, while you write to `/p/{roomId}/{memberId}`) fires again **immediately and optimistically** with your own just-written value — before any server round-trip. The SDK does not distinguish "changed because I wrote it" from "changed because a peer wrote it" at the callback level.
* **Not a bandwidth cost:** this is a purely local callback re-firing off the already-synchronized in-process cache — no extra bytes cross the wire. Where it matters, it's a data-correctness concern, not an efficiency one.
* **Governing rule: self-echo protection is only needed where uplink and downlink share the same store.** `Room` and `Tactical` each have exactly one store (`Set()`/`Get()` — see [`COMPANION_DATA_SYNC_MODEL.md` §0](COMPANION_DATA_SYNC_MODEL.md)) written by both this device's own uplink and its own Observer's downlink, closing a loop a self-write can travel around and come back through. `w2p Telemetry` does not: it only ever relays *other* players' telemetry, never the local device's own, so there's no shared-store loop to protect against. This is why the three channels are no longer treated uniformly:
  * **Telemetry — no self-echo filtering.** `handleTelemetryChildUpsert`'s pipeline is decrypt → sanitize/remove-out-of-order → canonicalize → `Set()` on `w2p Telemetry`, with no author check at any stage. A device's own position is never routed through this channel to begin with (it goes directly from local GPS into `local player management`'s `self` — see §F below), so there is nothing for a self-echo to overwrite here.
  * **Room — no self-echo filtering.** The former `if id == localId { continue }` exclusion and re-insert-self-afterward logic (previously in `mergeRemoteMembers`) is removed entirely, not preserved or generalized. Self's own row, when it appears in the downloaded `/r/{roomId}` snapshot (echoed or server-confirmed, no distinction made), is accepted like any other row and `Set()` on `Room` as-is — its *appearance* is the positive signal that the join/host write actually succeeded server-side, not something defensive to filter out. **Accepted, documented tradeoff:** editing your own callsign/role twice in quick succession can show a brief, cosmetic flicker back to the first edit's stale value until its echo is overtaken by the second edit's — judged acceptable since identity fields aren't edited as rapidly or repeatedly as tactical markers get moved. Two reasons this is safe to accept rather than needing to be engineered around: (1) Firebase only ever learned your identity from you, so its echo can structurally never be *more* correct than what you already hold, only equal or stale — there is no "waiting for external confirmation" value here the way there legitimately is for a tactical placement; (2) `Room` staying live and self-inclusive (not excluded/frozen to a fixed local value) is what makes a future "commander reassigns a player's role remotely" feature possible at all — an excluded/frozen self row could never observe a server-driven role change.
  * **Tactical — self-echo filtering kept, but re-implemented.** Placing or moving a tactical indicator writes it to Firebase; since this device is also listening on the same path, its own write always echoes back through the same Observer pipeline. Two independent reasons the echo can't be blindly trusted: (1) **staleness, not corruption** — the round trip isn't instantaneous, so a *newer* local `Set()` that lands before an *older* write's echo arrives would, if the echo were applied blindly, overwrite current data with the device's own past data (a plain value-equality no-op check doesn't catch this, since the stale and current values are *different*, not equal); (2) Firebase delivery isn't guaranteed atomic per-record, so a listener can fire on a partially-settled write. The former `unacknowledgedIndicators` buffer and ACK-matching mechanism is removed entirely and replaced with a simpler, bidirectional comparison reusing the same "newest wins" principle used everywhere else in this design: for each incoming indicator, the canonicalize step compares its existing `TacticalIndicator.timestamp` field ([`TacticalIndicator.swift:186`](../RadarMap/Models/TacticalIndicator.swift#L186) — already present, no schema change) against the currently-held indicator's timestamp (read from `Tactical` — this is the genuinely bidirectional step) and keeps whichever is newer, before `Set()`. This `timestamp` field is ordinary domain content, returned by `Get()` like any other field — it is a distinct mechanism from `LS_data`'s own private per-structure `_ts` (§0 of the companion doc), which operates at a different layer (WCSession phone↔watch convergence) and is never exposed via `Get()` or compared against it.
* **Paired requirement — deterministic canonicalize, not just author filtering:** "did a peer actually change this" is only a meaningful question if identical content is guaranteed to compare equal to itself. `Room`'s roster and `Tactical`'s indicator set (the same native `[SquadMember]`/`[TacticalIndicator]` arrays that also feed the `*_ls` wire payload once encoded — see [`COMPANION_DATA_SYNC_MODEL.md` §3](COMPANION_DATA_SYNC_MODEL.md#3-merge-engine--conflict-resolution-rules)) are compared via native `Equatable` array `==`, not a serialized string, so encoder key-order/float-formatting nondeterminism can't affect the comparison at all — but `Array` equality is still order-sensitive, so the producing side must still emit a canonical, stable element order: the Room pipeline's canonicalize step sorts the roster by `id`, and Tactical's keeps indicators sorted by `timestamp`. Without this, Tactical's self-echo filtering alone would be insufficient — a filtered-clean rebuild from unordered source data (e.g. dictionary values) can still register as "changed" purely from element-order nondeterminism, which was observed as a permanent 1 Hz retransmission loop that never converged back when this comparison was done via a JSON-encoded string.
...
* **Guarantees:**
  * **Uplink Control (`GameStateManager.hasNetworkOwnership`):** `login_cycle != inactive` (`host_active OR join_active`) AND the per-platform lease condition — Watch is permitted whenever a session is actually active (`True` beyond the login-active check); Phone additionally requires the Watch lease to be expired (`phone.time > w2p_hs.active_until`). Enforced uniformly at every write call site into `FirebaseSyncManager` (`broadcastLocalTelemetry`, `setDead`, `updateLocalMember`) — a device that hasn't actually joined/hosted a session never writes, regardless of lease state, and no write path bypasses the check.
  ...
  * **Refresh Cadence:** Telemetry re-sent at the scheduled refresh interval every $10 \times T$ heartbeats even absent a qualifying delta, to assure cloud data freshness.
...
* **Self-echo consequence:** moving these writes onto the shared SDK connection is what triggers the self-echo behavior documented in §5.B ("Self-Echo Filtering on SDK Listener Callbacks: Per-Domain, Not Universal") — every write made here optimistically re-fires this device's own downstream listeners. Only `Tactical` needs a filtering response to that; `Room`'s and Telemetry's uplinks tolerate their own echo by design (§5.B), not because of an independent mitigation applied here.
...
### F. Display Classification: Membership × Telemetry Confirmation Gate (Other Players), and the Separate `self` Path

* **Components:** [`FirebaseSyncManager.swift`](../RadarMap/Managers/FirebaseSyncManager.swift) (`isMemberConfirmed(_:)`, `observedRosterMemberIds`, `observedTelemetryMemberIds`), [`GameStateManager.swift`](../RadarMap/Managers/GameStateManager.swift) (`updateOtherSquadMembers`).
* **`otherSquadMembers`/`Player annotations` no longer includes Self.** Self is split into its own path entirely — a `local player management` process combines position read **directly from local GPS** (no Firebase round-trip; self's own position is always locally known with certainty) with membership info (callsign, role) read via `Room.Get()` — not a value frozen at join time. This is deliberate: keeping self's role sourced live from `Room` rather than a local-only snapshot is what would let a future "commander reassigns a player's role remotely" feature reach the display at all. Self therefore never goes through the confirmation gate described below — there is no roster/telemetry wait for information this device already has with certainty. `otherSquadMembers` (and every view built from it) is other-players-only.
* **Problem the architecture change eliminates, structurally, for other players:** Room/Membership (§5.B, `/r/{roomId}/m/{memberId}`) and Remote Player Telemetry (`/p/{roomId}/{memberId}`) are two independent channels that can arrive in either order — the join path's roster write (`publishMemberToFirebase`) is fire-and-forget with no completion/retry, so a peer's telemetry frequently reaches this device before that peer's roster row does. Previously, a one-shot Firebase read triggered from telemetry (`fetchMemberDetails`) wrote directly into `activeRoom.members`, a store that also received writes from the persistent roster listener — the same shared-mutable-dictionary shape that produced the blanked-callsign bug this whole redesign traces back to (see `CLOUD_DATA_MANAGEMENT.md`'s sibling doc's Context section). **That one-shot fetch, and the `activeRoom.members` store it wrote into, are both deleted entirely, not hardened** — the Telemetry Observer pipeline now writes exclusively into `w2p Telemetry`, and the Room Observer pipeline writes exclusively into `Room`; a member's roster identity and a member's telemetry no longer live in — or contend for — the same store at all, so there is no longer a race between them to guard against at the data layer.
* **The confirmation gate itself is kept, unchanged in requirement, now reading from the collapsed stores:** `FirebaseSyncManager.isMemberConfirmed(_:)` remains the single gate for *other* players — `observedRosterMemberIds.contains(id) && observedTelemetryMemberIds.contains(id)` — and failing either check still excludes the member from `otherSquadMembers` entirely, with no fallback to a guessed/default classification. This is a real, resolved design decision, not a simplification left on the table: even though the original stub-clobbering race is gone, a member can still be roster-confirmed (present in `Room`) while having sent zero telemetry yet, and that distinction is still worth gating display on. `observedRosterMemberIds` is now maintained purely from the Room Observer's `Set()` calls (there is no second writer, e.g. a one-shot fetch, left to maintain it from); `observedTelemetryMemberIds` is maintained from either telemetry source, direct or relayed (see next bullet).
* **`observedTelemetryMemberIds` must track both telemetry sources identically, not just this device's own listener.** Per [`COMPANION_DATA_SYNC_MODEL.md` §0](COMPANION_DATA_SYNC_MODEL.md), `*_hs` is two independent unidirectional, unmerged streams — a device without its own `/p/{roomId}` listener attached (either the Phone while the Watch holds network ownership, or symmetrically the Watch while the Phone holds it, §5.C) instead receives the same telemetry relayed via whichever `*_hs.remotePlayerTelemetryJson` is inbound to it (`w2p_hs` for the Phone, `p2w_hs` for the Watch — read inline by role in `WatchConnectivityManager.handleIncomingApplicationContext`, `WatchConnectivityManager.swift:499,507-508`, which fires `onHighSpeedTelemetryReceived`), and both sources ultimately reach `w2p Telemetry` via `Set()`. `handleTelemetryChildUpsert` (the direct `/p/{roomId}` listener path) inserts into `observedTelemetryMemberIds` on every child event; `validateAndProcessPackets` (the relay entry point, called from `onHighSpeedTelemetryReceived` on either platform) does the same for every packet it processes, unconditionally (regardless of whether the packet itself passes freshness) — mirroring the direct-listener path exactly, fixed once in the shared function rather than per-caller. Without this, a device correctly relying on the relay (because it has no listener of its own attached, by design) could never satisfy `isMemberConfirmed` for *any* remote member, even though the roster (`Room.Get()`) showed them fine.
* **Trade-off, accepted:** a brand-new joiner is briefly invisible (rather than briefly wrong-colored) between their telemetry arriving and their roster row confirming — bounded by the normal convergence timing of the Room/Telemetry Observer pipelines, not by any timeout logic.
...
* **Room id (derived, the actual path key):** `FirebaseSyncManager.deriveRoomId(name:pin:)` = `name + deriveRoomPadding(pin, name, length: 16 - name.count)`, **always 16 characters total** (`AppConstants.UI.maxRoomNameLength`, `RadarPlayerSimulator.MAX_ROOM_NAME_LENGTH`, enforced server-side by `database.rules.json`'s `$roomId.length <= 16 && $roomId.matches(/^[A-Z0-9]+$/)` validation). The dynamic padding (4 to 12 characters, filling the remainder to 16) is drawn from the same 32-symbol Crockford Base32 alphabet used for member/indicator ids, via a domain-separated SHA-256 hash of `(name, pin)` (`FirebaseSyncManager.deriveRoomPadding`, mirrored in `RadarPlayerSimulator.derive_room_padding`) — see §7.A for the full alphabet, domain-separation rationale, and why every caller must go through `deriveRoomId` rather than inlining the formula.
...
The finalized schema, after the room-id hardening and short-key cleanup pass (dropping the earlier plain-room-name id, several dead fields, and REST-era polling). **`r/{roomId}`'s top-level fields (`id`, `hst`, `cap`, `mti`, `pin`) are now also carried on the WCSession-synced `Room` structure** (see [`COMPANION_DATA_SYNC_MODEL.md` §0/§2](COMPANION_DATA_SYNC_MODEL.md)), not only here plus a now-deleted `FirebaseSyncManager.activeRoom` copy — this is the same field set, one Firebase-side representation and one WCSession-side representation of it, kept in sync by the normal Room Observer pipeline/`Set()` path rather than a separate store.
...
`roomId = name + deriveRoomPadding(pin, name, length: 16 - name.count)`, exposed as the single combining function every caller must use:
...
* **One derivation, one call site pattern:** `hostRoom`, `joinRoom` (the manual-entry branch), and `GameStateManager.adoptCompanionSession` (the phone↔watch companion-adoption path — see [`COMPANION_DATA_SYNC_MODEL.md`](COMPANION_DATA_SYNC_MODEL.md)) all call `deriveRoomId` rather than inlining `name + deriveRoomPadding(...)` themselves. This was previously three independent inlinings, and `adoptCompanionSession`'s copy had silently drifted to omit the padding step entirely (`cleanId = roomName.uppercased()`, no padding) — meaning that when the low-speed convergence handler fired `adoptCompanionSession` reflexively (including on the device that already owned the room, e.g. the very first WCSession context exchange after the watch app launches), it derived the *wrong, unpadded* id, called `connectToExistingRoom` with it, and that function's own `activeRoom?.id != cleanId` check (§ correctly designed in isolation) then replaced the real, populated room with a brand-new empty one at the wrong path — while the still-correctly-attached `/r/{realRoomId}/m` roster listener kept feeding real membership (but no telemetry, which was now polling the wrong path) back into it, producing a room whose members list looked fine but whose live positions were dead. Fixed by consolidating all three call sites onto the one `deriveRoomId` function so this class of drift can't recur; see the git history for the specific commit.
...
2. **Post-sync, full-room sweep** (`GameStateManager.enforceTacticalIndicatorMaintenance`, no host gating): runs on every `Tactical` update (including the one triggered by the client's own publish above), sorting the full merged set of indicators from `Tactical.Get()` — everyone's markers, not just this device's — by timestamp and evicting the oldest overflow. This is what closes the gap layer 1 can't: a client can't delete-before-add markers placed by other members that it doesn't know about yet, so cross-member overflow is corrected immediately *after* it becomes visible instead. The limit itself (`mti`) is read separately from `Room.Get()` — two distinct reads, matching the diagram's split between where the cap value lives (`Room`) and where the content it's capping lives (`Tactical`).
