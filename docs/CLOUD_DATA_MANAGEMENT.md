...
* **Custom Database URL → `AppConstants.Network.sanitizeInput`**: this field can't be restricted to alphanumerics — a real URL needs `: / . -` at minimum — so instead it's restricted to the full RFC 3986 URI character set (unreserved + reserved + percent-encoding) and control characters/whitespace/non-ASCII text are stripped. Wired into the shared `DatabaseURLField` component (`RadarMap/Views/Room/DatabaseURLField.swift`) so all three call sites (Settings, Create Room, Join) inherit it from one place, including text arriving from the live-camera OCR scanner. Sanitization here only guards against garbage characters that would otherwise reach `Database.database(url:)` (which traps on unparseable input) — it doesn't replace `AppConstants.Network.isValidDatabaseURL`'s well-formedness check, which still gates whether host/join can proceed.
* **Why not sanitize *every* text field the same way:** Callsign is stored purely as a JSON value (`csn`), never a path segment, so it only needs trim + uppercase — no character-set restriction is required for Firebase correctness, and overly restricting it would just annoy users choosing a callsign. Sanitization scope is chosen per field by how the value is used downstream, not applied uniformly for its own sake.
* **Server-side backstop:** `database.rules.json`'s `$roomId.matches(/^[A-Z0-9]+$/)` validation mirrors the client-side room-name/id character restriction, so a room id can't reach the database with an illegal or non-ASCII character even if some future client build skips the sanitizer.
...
The Cloud Function below (`pruneExcessTacticalIndicators`) mirrors this same oldest-by-timestamp logic and remains deployed as a backup for cases the client-side logic can't cover — an offline/stale client, or any client build that predates this logic — and is a no-op in the common case where clients already kept the branch at or under cap:
