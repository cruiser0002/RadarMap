...
* **`fresh_until` has been removed from `*_hs` entirely** (`PhoneToWatchHighSpeed`/`WatchToPhoneHighSpeed`, `WatchConnectivityManager.latestRemoteHSFreshUntil`, and the `onHighSpeedTelemetryReceived`/`onHighSpeedHeartRateReceived` closure signatures) — it was dead code, written on every publish but read by no consumer anywhere in the app. Every freshness/role decision already used `active_until` instead; see the model doc's §2 constants table.
