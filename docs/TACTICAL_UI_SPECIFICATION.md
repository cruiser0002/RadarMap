...
| **§3 Tactical Scales** | Scale Range Bounds | `1.0m` min / `2,500.0m` max | Clamping boundaries for Digital Crown, Radar-view zoom, and iOS Standard map pinch release via `MKMapView.CameraZoomRange` (§5.D.3) |
...
* **Native Follow with CameraZoomRange Altitude Lock:** Governed by native `showsUserLocation = true` / `userTrackingMode = .follow`. Altitude stability is enforced by setting a persistent `MKMapView.CameraZoomRange(minCenterCoordinateDistance: d, maxCenterCoordinateDistance: d)` where `min == max == d`, calculated via `AppConstants.UI.RadarScale.cameraDistance(forScale:)`. Because MapKit enforces this range clamp against its own internal camera updates, `.follow` tracking cannot drift altitude with movement speed or pop to default distances upon re-centering, completely avoiding the need to fight MapKit with manual `setCamera` calls.
...
* **Pinch-to-Zoom — Native During Gesture, Snaps on Release:** While a pinch gesture is actively in flight (`.began`), the camera zoom range is relaxed to `[minScale, maxScale]`, allowing fully continuous and native pinch manipulation. Upon gesture completion (`.ended` or `.cancelled`), the live ruler scale is read from geometry (`currentRulerScale`), snapped to the nearest discrete decade scale on the `[1, 2.5, 5]` ladder via `snapToDiscreteScale`, and the `CameraZoomRange` is re-clamped with `min == max` at the target distance (non-animated, since the live pinch has already settled the camera visually close to the target). If the user was following prior to pinching in place, `.follow` tracking is unconditionally restored.
* **Pinch Never Registers as a Pan:** Because MapKit zooms toward the pinch's focal point rather than the map center, a pinch can shift `mapView.centerCoordinate` well past `centerThresholdMeters` from the user's real position even when the user never intended to pan. Pinch release therefore never dispatches a `.pan` action from that incidental drift — only an explicit drag gesture can transition `MapTrackingState` to `.unlocked`.
* **HUD Lock-Button Confirmation (`isMapFollowConfirmed`):** The bottom-left Center Map button's icon is driven by `gameState.showsAsCenterLocked`, which requires both app-level intent (`mapCenterLockState.isLocked`) *and*, on the iOS map presentation, confirmation that native `.follow` tracking actually engaged. Outside of a pinch, that confirmation is asynchronous — `recenterOnUser` optimistically marks it unconfirmed and only flips it true once `mapView(_:didChange:)` reports `.follow`, so the button can't lie and show "locked" while the camera has silently stopped following. On pinch release specifically, confirmation is set immediately rather than waiting on that delegate callback, since the camera is already sitting at the exact target distance and tracking never actually left `.locked` — re-engaging `.follow` there is a formality, not a real transition.
...
| **Gestures** | Gesture | Standard pan/drag, tap, and native pinch-to-zoom gestures | [`StandardMapView.swift`](../RadarMap/Views/Map/StandardMapView.swift)<br>[`TacticalMKMapView.swift`](../RadarMap/Views/Map/iOS/TacticalMKMapView.swift) | • iPhone/iPad (`TacticalMKMapView`): native continuous MapKit pan/pinch-zoom during gesture, snapping cleanly to discrete decade ladder on release via `CameraZoomRange` re-clamp (§5.A, §5.D.3).<br>• watchOS (`StandardMapView`'s `NativeSwiftUIMapView`): Native `.interactionModes: .pan`; zoom is Digital-Crown-driven and snaps to the discrete `[1, 2.5, 5]` ladder.<br>• Drag gesture transitions state from `.locked` to `.unlocked` (panning) on both platforms.<br>• Tap gesture handles indicator placement when menu is pending. |
| **Crown / Pinch Zoom** | Zoom | Digital Crown & iOS pinch: discrete decade levels `[1, 2.5, 5]` | [`AppConstants.swift`](../RadarMap/AppConstants.swift)<br>[`TacticalRadarMapView.swift`](../RadarMap/Views/Map/TacticalRadarMapView.swift)<br>[`StandardMapView.swift`](../RadarMap/Views/Map/StandardMapView.swift)<br>[`TacticalMKMapView.swift`](../RadarMap/Views/Map/iOS/TacticalMKMapView.swift) | • Digital Crown rotation (watchOS) steps through discrete minor scales `[1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 2500.0]`, with altitude calculated via MapKit camera FOV trigonometry (`cameraDistance(forScale:)`).<br>• iOS pinch-to-zoom (`TacticalMKMapView`) allows continuous zoom during gesture and snaps to the identical discrete ladder on release, locking altitude via `CameraZoomRange`. |
...
2. **Camera Altitude & Aspect Ratio**: On watchOS and iOS, MapKit camera altitude is bound to the tactical scale via `AppConstants.UI.RadarScale.cameraDistance(forScale:)` with FOV tangent trigonometry ($V = 2 \cdot \text{altitude} \cdot \tan(15^\circ)$). On iOS, this altitude is locked continuously via `MKMapView.CameraZoomRange(minCenterCoordinateDistance: d, maxCenterCoordinateDistance: d)` where `min == max == d`.
3. **Decade Zoom Progression & Post-Zoom Snapping**: Zoom levels are strictly constrained to the $1 \to 2.5 \to 5$ decade sequence across metric ranges ($1\text{m}, 2.5\text{m}, 5\text{m}, 10\text{m}, 25\text{m}, 50\text{m}, 100\text{m}, 250\text{m}, 500\text{m}, 1000\text{m}, 2500\text{m}$) across both platforms. On watchOS, Digital Crown steps discretely through this ladder. On iOS Standard Map (`TacticalMKMapView`), pinch-to-zoom is continuous during the gesture, and upon gesture release the view reads the live ruler scale, snaps to the nearest discrete decade scale in `[1, 2.5, 5]`, and clamps `CameraZoomRange` with `min == max` to lock the camera at the target distance.
4. **Non-Destructive Centering**: Re-centering to the local user resets panning coordinates but does not touch zoom. On watchOS and in the Radar view this means the current discrete scale is explicitly retained; on the iPhone/iPad Standard map view the persistent `CameraZoomRange` constraint prevents MapKit from modifying altitude during tracking mode re-engagement.
5. **Native MapKit Follow-Me Mode (60Hz Smooth Tracking)**: In `StandardMapView` (watchOS), camera tracking when `trackingState.isLocked` is `true` uses native SwiftUI MapKit `MapCameraPosition.userLocation(fallback: .camera(...))`. The iPhone/iPad `TacticalMKMapView` adapter uses UIKit `userTrackingMode = .follow` combined with the persistent `CameraZoomRange` clamp to ensure altitude remains strictly invariant across speed variations and recentering transitions.
...
| **Center Map** | Bottom-Left | `location.fill` (locked) / `location` (unlocked) | Re-locks tracking to local user (`.locked`). **Strictly preserves active scale/altitude** (never resets to 50m). Icon reflects `gameState.showsAsCenterLocked` — on iOS this requires confirmed native `.follow` engagement, not just intent (see §5.A). |
...
- [ ] iPhone/iPad Standard map view (`TacticalMKMapView`) pinch-to-zoom is continuous during gesture, snaps to the nearest discrete decade scale on release, and re-clamps `CameraZoomRange` to lock altitude (§5.A, §5.D.3).
